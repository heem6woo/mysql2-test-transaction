...
- **Safety First**: 운영 DB 접속 시 자동 차단 (NODE_ENV + DB 이름 검증)
...
| `dbNameMustInclude` | `string` | `'test'` | DB 이름 필수 포함 문자열 |
| `allowedNodeEnv` | `string[]` | `['test']` | 허용된 NODE_ENV 값 |
...
첫 `start()` 호출 시 다음을 자동 검증합니다:

1. **NODE_ENV 검증**: 허용되지 않은 환경에서 실행 차단
2. **DB 이름 검증**: 테스트 DB가 아닌 경우 실행 차단

```
NODE_ENV=production             → Error
database=myapp_prod             → Error
NODE_ENV=test, database=myapp_test → OK
```
